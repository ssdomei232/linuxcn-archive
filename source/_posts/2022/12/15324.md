...
id: 15324
islctt: true
banner_img: /data/attachment/album/202212/07/133637yq2526zsp7f1t7a2.jpg
permalink: /article-15324-1.html
index_img: /data/attachment/album/202212/07/133637yq2526zsp7f1t7a2.jpg.thumb.jpg
related: []
reviewer: wxy
selector: lkxed
...
![Image of 6 bowls of different colored sindex_imges, and an arrow pointing to the second image of all the sindex_imges blended into one bowl.](/data/attachment/album/202212/07/133811dxvzpbh3by43twxl.jpg)
...
* <ruby> 选用 <rt>  index_imgk </rt></ruby>：按原样包含
...
### 接下来学习 Git cherry-index_imgk 命令吧


大多数开发人员将修改提交到某一分支上，但是之后发现他们一直**提交到了错误的分支上**。理想情况下，他们可以拿走那个提交，然后把它移到正确的分支，这正是 `git cherry-index_imgk` 命令的作用。


`git cherry-index_imgk` 命令利用了变基单个提交的方法。这一用法非常常见，以至于有了它自己的命令。


![Image of a woman index_imgking a cherry from one tree and putting on another tree.](/data/attachment/album/202212/07/133821chbpnu55pqp1pbbp.jpg)


要使用 `git cherry-index_imgk`，你只需告诉 Git 你要移动到“那个分支”的提交 ID（由 `HEAD` 指向）：



```
$ git cherry-index_imgk <target-ref>
...
```
$ git cherry-index_imgk -i 2bc01cd
...
hint: You can instead skip this commit with "git cherry-index_imgk --skip".
hint: To abort and get back to the state before "git cherry-index_imgk",
hint: run "git cherry-index_imgk --abort".
$ git cherry-index_imgk --abort
