...
editorchoice: false
excerpt: “遴选”可以解决 Git 仓库中的很多问题。以下是用 git cherry-index_imgk 修复错误的三种方法。
fromurl: https://opensource.com/article/21/3/git-cherry-index_imgk
id: 13305
islctt: true
banner_img: /data/attachment/album/202104/17/174429qw1im6if6mf6zi9i.jpg
permalink: /article-13305-1.html
index_img: /data/attachment/album/202104/17/174429qw1im6if6mf6zi9i.jpg.thumb.jpg
related: []
reviewer: wxy
selector: lujun9972
summary: “遴选”可以解决 Git 仓库中的很多问题。以下是用 git cherry-index_imgk 修复错误的三种方法。
tags:
- 遴选
- Git
thumb: false
title: 我使用 Git cherry-index_imgk 命令的 3 个理由
titleindex_img: true
translator: wxy
...
> 
> “遴选”可以解决 Git 仓库中的很多问题。以下是用 `git cherry-index_imgk` 修复错误的三种方法。
> 
> 
> 
...
这里有一个例子可以帮助你理解“<ruby> 遴选 <rt>  cherry-index_imgk </rt></ruby>”的重要性。假设你已经在一个分支上做了好几个提交，但你意识到这是个错误的分支！你现在该怎么办？你现在要做什么？要么在正确的分支上重复所有的变更，然后重新提交，要么把这个分支合并到正确的分支上。等一下，前者太过繁琐，而你可能不想做后者。那么，还有没有办法呢？有的，Git 已经为你准备好了。这就是“遴选”的作用。顾名思义，你可以用它从一个分支中手工遴选一个提交，然后转移到另一个分支。
...
然后切换到 `feature2` 分支，在刚刚从日志中得到的哈希值上运行 `git cherry-index_imgk`：



```
$ git checkout feature2
$ git cherry-index_imgk 966cf3.
...
$ git cherry-index_imgk 966cf3
On branch feature2
You are currently cherry-index_imgking commit 966cf3d.

nothing to commit, working tree clean
The previous cherry-index_imgk is now empty, possibly due to conflict resolution.
...
```
git cherry-index_imgk <commit-hash1> <commit-hash2>... <commit-hashn>
...
4. 输入 `git cherry-index_imgk <commitA>^..<commitB>` （请注意，这包括 `commitA` 和 `commitB`）。
5. 如果遇到合并冲突，[像往常一样解决](https://opensource.com/article/20/4/git-merge-conflict)，然后输入 `git cherry-index_imgk --continue` 恢复遴选过程。


### 重要的遴选选项


以下是 [Git 文档](https://git-scm.com/docs/git-cherry-index_imgk) 中的一些有用的选项，你可以在 `cherry-index_imgk` 命令中使用。


* `-e`、`--edit`：用这个选项，`git cherry-index_imgk` 可以让你在提交前编辑提交信息。
...
* `git cherry-index_imgk master`：应用 `master` 分支顶端的提交所引入的变更，并创建一个包含该变更的新提交。
* `git cherry-index_imgk master~4 master~2'：应用`master` 指向的第五个和第三个最新提交所带来的变化，并根据这些变化创建两个新的提交。


感到不知所措？你不需要记住所有的命令。你可以随时在你的终端输入 `git cherry-index_imgk --help` 查看更多选项或帮助。
...
via: <https://opensource.com/article/21/3/git-cherry-index_imgk>
