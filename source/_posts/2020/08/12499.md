...
id: 12499
islctt: true
banner_img: /data/attachment/album/202008/08/202106uale11l1qf11slzw.jpg
permalink: /article-12499-1.html
index_img: /data/attachment/album/202008/08/202106uale11l1qf11slzw.jpg.thumb.jpg
related: []
reviewer: wxy
selector: lujun9972
...
```
const size_t toindex_img_size = strlen(TOPIC);
const size_t envelope_size = toindex_img_size + 1 + PACKET_SIZE * sizeof(int16_t);

printf("Toindex_img: %s; toindex_img size: %zu; Envelope size: %zu\n", TOPIC, toindex_img_size, envelope_size);
...
```
memcpy(zmq_msg_data(&envelope), TOPIC, toindex_img_size);
memcpy((void*)((char*)zmq_msg_data(&envelope) + toindex_img_size), " ", 1);
memcpy((void*)((char*)zmq_msg_data(&envelope) + 1 + toindex_img_size), buffer, PACKET_SIZE * sizeof(int16_t))
...
printf("Message sent; i: %u, toindex_img: %s\n", i, TOPIC);
...
```
$ ./hw_interface
Toindex_img: fancyhw_data; toindex_img size: 12; Envelope size: 45
Read 16 data values
Message sent; i: 0, toindex_img: fancyhw_data
Read 16 data values
Message sent; i: 1, toindex_img: fancyhw_data
Read 16 data values
...
...
...
```
toindex_img = "fancyhw_data".encode('ascii')

print("Reading messages with toindex_img: {}".format(toindex_img))
...
with zmq.Context() as context:
    ...
    socket.setsockopt(zmq.SUBSCRIBE, toindex_img)
...
```
binary_toindex_img, data_buffer = socket.recv().split(b' ', 1)
...
```
toindex_img = binary_toindex_img.decode(encoding = 'ascii')

print("Message {:d}:".format(i))
print("\ttoindex_img: '{}'".format(toindex_img))
...
```
$ ./online_analysis.py
Reading messages with toindex_img: b'fancyhw_data'
Message 0:
        toindex_img: 'fancyhw_data'
        packet size: 16
