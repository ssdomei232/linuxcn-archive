...
id: 12407
islctt: true
banner_img: /data/attachment/album/202007/12/100823rj096h3ax4hhehey.jpg
permalink: /article-12407-1.html
index_img: /data/attachment/album/202007/12/100823rj096h3ax4hhehey.jpg.thumb.jpg
related:
- displayorder: 0
  raid: 12371
reviewer: wxy
...
启用服务后，请配置数据源，客户端将从该数据源查询配置数据。有[许多数据源类型](https://cloudinit.readthedocs.io/en/latest/toindex_imgs/datasources.html)，而且大多数都是为特定的云提供商配置的。对于你的家庭实验室，请使用 NoCloud 数据源，（如上所述）它是为在没有云提供商的情况下使用 Cloud-init 而设计的。
...
在这里，Cloud-init 请求的是[实例元数据](https://cloudinit.readthedocs.io/en/latest/toindex_imgs/instancedata.html#what-is-instance-data)信息。这个文件可以包含很多关于实例本身的信息，例如实例 ID、分配实例的主机名、云 ID，甚至网络信息。
...
你可以使用[许多 user-data 模块](https://cloudinit.readthedocs.io/en/latest/toindex_imgs/modules.html)来配置你的实例。对于这个例子，只需使用 `write_files` 模块在客户端创建一些测试文件，并验证 Cloud-init 是否工作。
