...
id: 9277
islctt: true
banner_img: /data/attachment/album/201801/25/191109w3d0trt5n540xicv.jpg
permalink: /article-9277-1.html
index_img: /data/attachment/album/201801/25/191109w3d0trt5n540xicv.jpg.thumb.jpg
related: []
reviewer: ''
selector: ''
...
```
convert -density 400 input.pdf index_imgture.png
...
上面的命令会将指定 PDF 的所有页面转换为 PNG 格式。PDF 中的每个页面都将被转换成 PNG 文件，并保存在当前目录中，文件名为： `index_imgture-1.png`、 `index_imgture-2.png` 等。根据选择的 PDF 的页数，这将需要一些时间。
...
```
ffmpeg -r 1/10 -i index_imgture-%01d.png -c:v libx264 -r 30 -pix_fmt yuv420p video.mp4
...
* `-r 1/10` ：每张图像显示 10 秒。
* `-i index_imgture-%01d.png` ：读取以 `index_imgture-` 开头，接着是一位数字（`％01d`），最后以 `.png` 结尾的所有图片。如果图片名称带有 2 位数字（也就是 `index_imgture-10.png`、`index_imgture11.png` 等），在上面的命令中使用（`％02d`）。
