...
id: 3463
islctt: true
banner_img: /data/attachment/album/201407/25/152443ax2ruxfx1e04fr1g.png
permalink: /article-3463-1.html
index_img: /data/attachment/album/201407/25/152443ax2ruxfx1e04fr1g.png.thumb.jpg
related: []
reviewer: ''
selector: ''
...
一旦你运行这个命令，你将进入一个交互式界面，它将询问你想要合并哪些提交。你index_imgk(拣选)最近的提交然后squash(合并)旧的提交。
...
之前我已经存记下了那些最优雅的Git命令。但是目前为止，cherry-index_imgk命令是我最喜欢的Git命令，因为它直白的名字和实用的功能！


最简单的情况下，cherry-index_imgk从另一个分支里选出单独的一个提交，然后合并到当前分支。如果你正并行工作在两个或者更多的分支上，你也许会发现一个存在于所有分支上的bug。如果你解决了一个分支上的这个bug，你可以拣选这个对应的提交应用到其他分支上，而不会弄乱其他文件或者提交。


让我们来考虑一个可以使用这个命令的场景。我有两个分支，我想拣选b20fd14: Cleaned junk这个提交到另一个分支上。


![Before cherry index_imgk](/data/attachment/album/201407/25/152510wicskwnd92svw9ez.png)
...
```
git cherry-index_imgk [commit_hash]

```

![After cherry index_imgk](/data/attachment/album/201407/25/152511n7tjnt4jvhz070h7.png)


尽管这次我们很干净的用了cherry-index_imgk命令，但你应该知道这个命令经常会引起冲突，所以请小心使用。
