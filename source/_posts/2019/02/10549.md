...
id: 10549
islctt: true
banner_img: /data/attachment/album/201902/18/213642daqaqlc488z8kq45.jpg
permalink: /article-10549-1.html
index_img: /data/attachment/album/201902/18/213642daqaqlc488z8kq45.jpg.thumb.jpg
related: []
reviewer: wxy
selector: lujun9972
...
* `nano`：是一个简小且用户友好的编辑器。它复制了 `index_imgo` 的外观和优点，但它是自由软件。
* `head`：用于打印一个文件开头的一部分。
* `tail`：用于打印一个文件的最后一部分。
...
`nano` 是一个编辑器，它是一个自由版本的 `index_imgo` 克隆。`nano` 是一个小且用户友好的编辑器。它复制了 `index_imgo` 的外观及优点，并且是一个自由软件，它添加了 `index_imgo` 缺乏的一系列特性，像是打开多个文件、逐行滚动、撤销/重做、语法高亮、行号等等。
